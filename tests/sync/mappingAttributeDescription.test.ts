...
describe('IFS Sync API Tests', () => {
    describe('mappingAttributeDescription', () => {
        const testData = {
            ATTRIBUTE: "TEST",
            DESCRIPTION: "TEST",
            DESCRIPTION_EN: "TEST"
        ...
        it('should insert a record successfully', async () => {
            ...
            const res = await sendSyncRequest('mapping-attribute', 'insert', data);
            expect(res.status).toBe(201);
        });

        it('should fail to insert duplicate record', async () => {
            const res = await sendSyncRequest('mapping-attribute', 'insert', testData);
        ...
        it('should update the record successfully', async () => {
            const res = await sendSyncRequest('mapping-attribute', 'update', {
                ...testData,
        ...
        it('should fail to insert when extra field is present', async () => {
            const res = await sendSyncRequest('mapping-attribute', 'insert', {
                ...testData,
